...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        operation_summary='Activate 2FA for the current user with a verify token',
        responses={
            '204': '2FA activated',
...
@method_decorator(
    name='post',
    decorator=swagger_auto_schema(
        operation_summary='Deactivate 2FA for the current user',
        responses={
            '204': '2FA deactivated',
            '401': 'Not authenticated',
        },
...
@method_decorator(
    name='get',
    decorator=swagger_auto_schema(
        operation_summary='Get information for showing the 2FA qr code',
        responses={
...
class Get2FAUrl(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        url = pyotp.totp.TOTP(secret).provisioning_uri(
            name=request.user.email, issuer_name=settings.TWOFACTOR_ISSUER
        )
